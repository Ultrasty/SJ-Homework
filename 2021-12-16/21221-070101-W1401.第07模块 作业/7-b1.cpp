...
struct tj_time* tj_time_convert(int input_time)
{
	...
	int& current_year = result.tj_year;
	int& current_month = result.tj_month;
	int& current_day = result.tj_day;

	for (int i = 0; i < past_day; i++) {
		current_day++;
		if (current_day > nian[is_run_nian(current_year)][current_month-1]) {
			current_day -= nian[is_run_nian(current_year)][current_month-1];
			current_month++;
		}
